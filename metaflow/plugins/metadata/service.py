...
class ServiceMetadataProvider(MetadataProvider):
    ...
    def compute_info(cls, val):
        ...
        try:
            resp = requests.get(
                os.path.join(v, "ping"), headers=SERVICE_HEADERS.copy(), timeout=60
            )
                ...
                if method == "GET":
                    if monitor:
                        with monitor.measure("metaflow.service_metadata.get"):
                            resp = requests.get(
                                url, headers=SERVICE_HEADERS.copy(), timeout=60
                            )
                    else:
                        resp = requests.get(
                            url, headers=SERVICE_HEADERS.copy(), timeout=60
                    ...
                    if monitor:
                        ...
                        resp = requests.post(
                            url, headers=SERVICE_HEADERS.copy(), json=data, timeout=60
                        )
                elif method == "PATCH":
                    if monitor:
        ...
        for i in range(SERVICE_RETRY_COUNT):
            try:
                if monitor:
                    ...
                    resp = requests.get(url, headers=SERVICE_HEADERS.copy(), timeout=60)
            except:
                if monitor:
                    with monitor.count("metaflow.service_metadata.failed_request"):
                        if i == SERVICE_RETRY_COUNT - 1:
